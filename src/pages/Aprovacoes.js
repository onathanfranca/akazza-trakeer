...
export default function Aprovacoes({ casas, users }) {
  ...
  useEffect(() => {
    ...
    let q;
    if (filtro === 'pendente') {
      q = query(collection(db, 'cpas'), where('status', '==', 'pendente'), orderBy('createdAt', 'desc'));
    } else if (filtro === 'aprovado') {
      q = query(collection(db, 'cpas'), where('status', '==', 'aprovado'), orderBy('createdAt', 'desc'));
    } else {
      q = query(collection(db, 'cpas'), where('status', '==', 'rejeitado'), orderBy('createdAt', 'desc'));
    }
  ...
  const counts = useMemo(() => {
    // Conta pendentes em tempo real via snapshot separado
    return {};
  }, []);
  ...
  useEffect(() => {
    const q1 = query(collection(db, 'cpas'), where('status', '==', 'pendente'));
    const q2 = query(collection(db, 'cpas'), where('status', '==', 'aprovado'));
    const q3 = query(collection(db, 'cpas'), where('status', '==', 'rejeitado'));
        ...
        {[
          ...
          >
            ...
            <span style={{
              ...
              ...(opt.key === 'pendente' && pendentesCount > 0 ? {
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6, flexShrink: 0 }}>
                    ...
                    <div style={{ display: 'flex', gap: 6 }}>
                      {filtro === 'pendente' && (
                        <>
                          <button
                            onClick={() => aprovar(cpa.id)}
                            disabled={salvando}
                            style={{
                              background: 'rgba(26,170,110,0.15)', border: '1.5px solid var(--green)',
                              borderRadius: 8, padding: '6px 12px', fontSize: 12, color: 'var(--green)',
                              cursor: 'pointer', fontWeight: 700, fontFamily: 'var(--font-body)',
                              transition: 'all .15s',
                            }}
                            onMouseEnter={e => e.target.style.background = 'rgba(26,170,110,0.3)'}
                            onMouseLeave={e => e.target.style.background = 'rgba(26,170,110,0.15)'}
                          >✅ Aprovar</button>
                          <button
                            onClick={() => { setRejeitandoId(isRejeitando ? null : cpa.id); setMotivo(''); }}
                            disabled={salvando}
                            style={{
                              background: 'rgba(229,57,53,0.15)', border: '1.5px solid var(--red)',
                              borderRadius: 8, padding: '6px 12px', fontSize: 12, color: 'var(--red)',
                              cursor: 'pointer', fontWeight: 700, fontFamily: 'var(--font-body)',
                              transition: 'all .15s',
                            }}
                            onMouseEnter={e => e.target.style.background = 'rgba(229,57,53,0.3)'}
                      ...
                      {filtro === 'aprovado' && (
                        <button
                          onClick={() => voltarPendente(cpa.id)}
                          disabled={salvando}
                          style={{
                            background: 'rgba(245,158,11,0.15)', border: '1.5px solid #f59e0b',
                            borderRadius: 8, padding: '6px 12px', fontSize: 12, color: '#f59e0b',
                            cursor: 'pointer', fontWeight: 700, fontFamily: 'var(--font-body)',
                          }}
                        >↩️ Pendente</button>
                      )}
                ...
                {isRejeitando && (
                  <div style={{
                    ...
                    <input
                      ...
                      style={{ marginBottom: 8, borderColor: 'rgba(229,57,53,0.4)' }}
                      autoFocus
                      onKeyDown={e => { if (e.key === 'Enter') rejeitar(cpa.id); if (e.key === 'Escape') { setRejeitandoId(null); setMotivo(''); } }}
                    />
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button
                        onClick={() => rejeitar(cpa.id)}
                        disabled={salvando || !motivo.trim()}
                        style={{
                          background: 'var(--red)', border: 'none', borderRadius: 8,
                          padding: '8px 16px', fontSize: 13, color: '#fff',
                          cursor: motivo.trim() ? 'pointer' : 'not-allowed', fontWeight: 700,
                          opacity: motivo.trim() ? 1 : 0.5, fontFamily: 'var(--font-body)',
                        }}
                      >{salvando ? 'Salvando...' : '❌ Confirmar rejeição'}</button>
                      <button
                        onClick={() => { setRejeitandoId(null); setMotivo(''); }}
                        style={{
